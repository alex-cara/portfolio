...
<div class="center h-full w-[66%] items-center justify-center pb-4">
	...
	<div>
		<p class="z-10 text-center text-sm">
			...
			document.
			<br />
			The program is a CLI tool that uses I/O from the file system (i.e. hard drive/ssd), processes it,
			and writes the encrypted or decrypted output. Since file I/O tends to be the main performance bottleneck,
			multithreading can only do so much. Despite this, I implemented a system that allows files to be
			prefetched and loaded while computation is being done on other files. This overlap between computation
			and I/O helps maximize performance within hardware limits, trying to optimize around the I/O bottleneck.
			...
			<br />
			<br />
			Here is a multithreaded file reader that allows prefetching of files, and calls the function for
			the computation and reading in files.
		</p>
	</div>
